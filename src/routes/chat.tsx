...
const DEMO_CLIPS = [
  {
    id: "c1", user: "Aryan Sharma", handle: "@aryanjee25", avatar: "A", color: "from-blue-500 to-cyan-500",
    caption: "Solved this optics problem in 90 seconds using the mirror formula trick 🪞✨ #JEE2025 #Physics",
    likes: 847, comments: 63, shares: 124, views: "12.4K", time: "3m ago", hasVideo: true, topic: "Physics", topicColor: "text-blue-400 bg-blue-400/10",
  },
  {
    id: "c2", user: "Meera Patel", handle: "@meerachemsit", avatar: "M", color: "from-green-500 to-teal-500",
    caption: "The BEST way to memorize hybridization states — my colour-coded notes system 📝🎨 Drop a ❤️ if this helped!",
    likes: 2341, comments: 189, shares: 412, views: "38.7K", time: "47m ago", hasVideo: true, topic: "Chemistry", topicColor: "text-green-400 bg-green-400/10",
  },
  {
    id: "c3", user: "Rohan Gupta", handle: "@rohangupta_iit", avatar: "R", color: "from-purple-500 to-pink-500",
    caption: "Day 312 of JEE prep. Mock score today: 287/300. The grind never stops 💪 #JEE2025 #Maths",
    likes: 5812, comments: 321, shares: 891, views: "94.2K", time: "2h ago", hasVideo: false, topic: "Maths", topicColor: "text-purple-400 bg-purple-400/10",
  },
  {
    id: "c4", user: "Siya Verma", handle: "@siyaverma_jee", avatar: "S", color: "from-orange-500 to-yellow-500",
    caption: "Rotational mechanics made easy — this single diagram covers 80% of JEE questions on the topic. Save this! 🔄",
    likes: 3129, comments: 244, shares: 678, views: "51.1K", time: "4h ago", hasVideo: true, topic: "Physics", topicColor: "text-blue-400 bg-blue-400/10",
  },
];

const DEMO_COMMUNITIES: Community[] = [
  {
    id: "com1", name: "JEE Advanced 2025",
    desc: "Official study community for JEE Advanced aspirants. Share notes, strategies & doubts.",
    members: 24800, avatar: "J", color: "from-primary to-chart-4",
    channels: ["announcements", "physics", "chemistry", "maths", "doubts", "motivation"],
    joined: true, verified: true,
  },
  {
    id: "com2", name: "IIT Delhi – Admits 2025",
    desc: "For students who got into IIT Delhi or are aiming for it. Connect before joining.",
    members: 8200, avatar: "D", color: "from-blue-500 to-indigo-500",
    channels: ["general", "hostel-life", "branch-advice", "memes"],
    joined: true, verified: true,
  },
  {
    id: "com3", name: "Fiitjee Students Hub",
    desc: "All Fiitjee students — share notes, test papers, and batch experiences here.",
    members: 41300, avatar: "F", color: "from-green-500 to-emerald-500",
    channels: ["general", "test-papers", "teacher-notes", "results"],
    joined: false, verified: false,
  },
  {
    id: "com4", name: "Night Owls Study Club",
    desc: "For those who study past midnight. 11PM–4AM study sessions with daily check-ins.",
    members: 6700, avatar: "N", color: "from-indigo-500 to-purple-500",
...
function CreateGroupDialog({
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      const { data: room, error } = await supabase.rpc("create_group", { _name: name.trim() });
      if (error) throw error;
      if (room) {
        for (const peerId of selectedMembers) {
          await supabase.from("room_members").insert({ room_id: room, user_id: peerId }).catch(() => {});
        }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="max-h-[180px] overflow-y-auto border border-border/40 rounded-xl p-2 space-y-1">
              {(conns.data ?? []).length === 0 && (
                <div className="text-xs text-muted-foreground px-2 py-3">No connections yet. Connect with others first.</div>
              )}
              {(conns.data ?? []).map((c) => {
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSelectedMembers((prev) => {
                        ...
                        checked ? next.delete(peerId) : next.add(peerId);
                        return next;
                      })
                    }
                    className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-surface-2/50 transition"
                  >
                    <div className={`size-5 rounded-md border ${checked ? "bg-primary border-primary" : "border-border"} grid place-items-center`}>
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        const { data: up, error } = await supabase.storage.from("chat_media").upload(path, file, {
          cacheControl: "3600",
          upsert: false,
        });
        if (error) throw error;
        ...
        <div className="space-y-4">
              ...
              {["Physics", "Chemistry", "Maths", "Motivation", "General"].map((t) => (
                <button
                  ...
                  className={`px-3 h-7 rounded-full text-xs font-medium transition ${
                    topic === t ? "bg-primary text-primary-foreground" : "bg-surface-2 text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {t}
                </button>
          ...
          <div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 h-20 rounded-xl border-2 border-dashed border-border/60 hover:border-primary/50 hover:bg-primary/5 transition text-muted-foreground"
            >
              {preview ? (
                ...
                <>
                  ...
                  <span className="text-sm">Tap to upload image or video</span>
                </>
              )}
            </button>
          </div>
...
function ChatMoreMenu({
  onClear, onMute, onBlock, onDelete,
}: {
  onClear: () => void;
  onMute: () => void;
  onBlock: () => void;
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={onMute}>
          <VolumeX className="size-3.5 mr-2" /> Mute notifications
        </DropdownMenuItem>
        <DropdownMenuItem onClick={onClear}>
          <Trash2 className="size-3.5 mr-2" /> Clear chat
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => toast.success("Chat pinned to top")}>
          <Pin className="size-3.5 mr-2" /> Pin to top
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={onBlock} className="text-red-400 focus:text-red-400">
          <ShieldAlert className="size-3.5 mr-2" /> Block
        </DropdownMenuItem>
        <DropdownMenuItem onClick={onDelete} className="text-red-400 focus:text-red-400">
          <Trash2 className="size-3.5 mr-2" /> Delete chat
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function ChatPage() {
  ...
  const visibleMsgs = (msgs.data ?? []).filter(
    (m) => !(m.deleted_by_users ?? []).includes(user?.id ?? "") && new Date(m.created_at).getTime() > clearedAt,
  );
    ...
    <div className="h-[calc(100vh-3.5rem)] lg:h-screen flex flex-col">
      ...
      <Dialog open={!!showImagePreview} onOpenChange={() => setShowImagePreview(null)}>
        <DialogContent className="max-w-3xl p-0 overflow-hidden bg-black/90 border-0">
          {showImagePreview && (
            <img src={showImagePreview} alt="Preview" className="w-full h-full object-contain max-h-[80vh]" />
          )}
        </DialogContent>
      </Dialog>
            ...
            <aside className={`${showChatPane ? "hidden lg:flex" : "flex"} w-full lg:w-80 xl:w-96 shrink-0 flex-col border-r border-border/60 bg-surface/20`}>
              <div className="px-4 py-3 border-b border-border/60 bg-surface/40 backdrop-blur-xl flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => fileRef.current?.click()} className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition">
                  ...
                  <DropdownMenu open={showNewChatMenu} onOpenChange={setShowNewChatMenu}>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => { setShowNewChatMenu(false); setShowCreateGroup(true); }}>
                        <Users className="size-3.5 mr-2" /> New group
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => { setShowNewChatMenu(false); setShowCreateCommunity(true); }}>
                        <Globe className="size-3.5 mr-2" /> New community
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => { setShowNewChatMenu(false); setShowCreateClip(true); }}>
                        <Film className="size-3.5 mr-2" /> New clip
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => { setShowNewChatMenu(false); setSearch(""); }}>
                        <Pencil className="size-3.5 mr-2" /> New chat
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => toast.info("Select a chat to pin")}>
                        <Pin className="size-3.5 mr-2" /> Select pinned chats
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => toast.info("Settings coming soon")}>
                        <Settings className="size-3.5 mr-2" /> Settings
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => toast.info("Archiving coming soon")}>
                        <FileText className="size-3.5 mr-2" /> Archived chats
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                    ...
                    {incoming.map((c) => {
                      ...
                      return (
                        <div key={c.id} className="flex items-center gap-2.5 glass-panel rounded-xl px-3 py-2">
                          ...
                          <button onClick={() => respond(c.id, "accepted")} className="size-7 rounded-md grid place-items-center bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 transition">
                            <Check className="size-3.5" />
                          </button>
                          <button onClick={() => respond(c.id, "blocked")} className="size-7 rounded-md grid place-items-center bg-red-500/20 text-red-400 hover:bg-red-500/30 transition">
                ...
                {accepted.length === 0 ? (
                  ...
                  accepted.map((c) => {
                    ...
                    return (
                      <button
                        key={c.id}
                        onClick={() => openChat(peerId, p?.display_name ?? "Aspirant", p?.current_streak ?? 0)}
                        className={`w-full flex items-center gap-3 px-4 py-3 transition border-b border-border/20 ${active ? "bg-surface-2/70" : "hover:bg-surface-2/40"}`}
                      >
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            ...
                            <span className="text-xs text-muted-foreground truncate">{p?.current_streak ?? 0} day streak</span>
                          </div>
                        </div>
                      </button>
                    );
              ...
              {!activePeer ? (
                ...
                <>
                  <header className="flex items-center gap-3 px-4 h-[62px] border-b border-border/60 bg-surface/50 backdrop-blur-xl shrink-0">
                    <button className="lg:hidden size-9 rounded-lg grid place-items-center text-muted-foreground hover:bg-surface-2 mr-1" onClick={() => setActivePeer(null)}>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <ChatMoreMenu
                        ...
                        onMute={() => toast.success("Notifications muted for this chat")}
                        onBlock={() => toast.success("User blocked")}
                        onDelete={() => { setActivePeer(null); setActiveRoom(null); toast.success("Chat deleted"); }}
                      />
                    </div>
                  </header>
                  ...
                  <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 lg:px-6 py-4 space-y-1">
                        ...
                        return (
                          <div key={m.id} className={`group flex ${mine ? "justify-end" : "justify-start"} ${prevMine ? "mt-0.5" : "mt-3"}`}>
                            <div className="flex items-end gap-1 max-w-[75%]">
                              ...
                              <div className={`relative px-3 py-2 text-sm shadow-sm ${
                                gone
                                ...
                                {gone ? (
                                  ...
                                  <>
                                    {m.message_type === "image" && m.media_url && (
                                      <button onClick={() => setShowImagePreview(m.media_url)} className="block mb-1">
                                        <img src={m.media_url} alt="Shared" className="rounded-lg max-w-[240px] max-h-[200px] object-cover" />
                                      </button>
                                    )}
                                    {m.message_type === "video" && m.media_url && (
                                      <div className="mb-1">
                                        <video src={m.media_url} className="rounded-lg max-w-[240px] max-h-[200px] object-cover" controls preload="metadata" />
                                      </div>
                                    )}
                    ...
                    {fileUploading && (
                      <div className="flex justify-end">
                        <div className="bg-surface-2/80 rounded-2xl rounded-br-sm px-3 py-2 text-sm flex items-center gap-2 text-muted-foreground">
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); send(); }} className="shrink-0 px-3 py-3 border-t border-border/40 bg-surface/40 backdrop-blur-xl flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => fileRef.current?.click()} className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition shrink-0">
          ...
          <>
            <aside className={`${showGroupPane ? "hidden lg:flex" : "flex"} w-full lg:w-80 xl:w-96 shrink-0 flex-col border-r border-border/60 bg-surface/20`}>
              <div className="px-4 py-3 border-b border-border/60 bg-surface/40 backdrop-blur-xl flex items-center justify-between shrink-0">
                ...
                <button onClick={() => setShowCreateGroup(true)} className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition">
              ...
              <div className="px-3 py-2.5 border-b border-border/40 shrink-0">
                <div className="relative">
                  <Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" />
                  <Input placeholder="Search groups…" className="pl-9 bg-surface-2/50 border-border/40 rounded-xl h-9 text-sm" />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto">
                {DEFAULT_CLIP_GROUPS.map((g) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 transition border-b border-border/20 ${activeGroup?.id === g.id ? "bg-surface-2/70" : "hover:bg-surface-2/40"}`}
                  >
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <div className="flex items-center justify-between mt-0.5">
                        ...
                        {g.unread > 0 && (
                          <span className="ml-2 shrink-0 min-w-[18px] h-[18px] rounded-full bg-primary text-primary-foreground text-[10px] font-bold grid place-items-center px-1">{g.unread}</span>
                        )}
                      </div>
                    </div>
                  </button>
              ...
              {!activeGroup ? (
                <div className="flex-1 grid place-items-center text-center px-6">
                  <div className="max-w-xs">
                    ...
                    <Button className="mt-4 rounded-xl" size="sm" onClick={() => setShowCreateGroup(true)}>
                ...
                <>
                  <header className="flex items-center gap-3 px-4 h-[62px] border-b border-border/60 bg-surface/50 backdrop-blur-xl shrink-0">
                    <button className="lg:hidden size-9 rounded-lg grid place-items-center text-muted-foreground hover:bg-surface-2 mr-1" onClick={() => setActiveGroup(null)}>
                      <ArrowLeft className="size-4.5" />
                    </button>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10.5px] text-muted-foreground">{activeGroup.members} members</div>
                    </div>
                    <div className="flex items-center gap-1">
                      <button className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition"><Phone className="size-4" /></button>
                      <button className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition"><Video className="size-4" /></button>
        ...
        {tab === "clips" && (
          <div className="flex-1 min-w-0 overflow-y-auto">
            <div className="sticky top-0 z-10 px-4 py-3 border-b border-border/60 bg-background/90 backdrop-blur-xl flex items-center justify-between">
              ...
              <button onClick={() => setShowCreateClip(true)} className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition">
                ...
                <article key={clip.id} className="bg-surface/30 sm:rounded-2xl border-y sm:border border-border/40 overflow-hidden">
                  <div className="flex items-center gap-3 px-4 pt-4 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[10.5px] px-1.5 py-0.5 rounded-md font-medium ${clip.topicColor}`}>{clip.topic}</span>
                      </div>
                  ...
                  <div className="flex items-center gap-1 px-3 py-2 border-t border-border/30">
                    <button onClick={() => setLikedClips((prev) => { const next = new Set(prev); next.has(clip.id) ? next.delete(clip.id) : next.add(clip.id); return next; })} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs transition ${likedClips.has(clip.id) ? "text-pink-500 bg-pink-500/10" : "text-muted-foreground hover:text-pink-500 hover:bg-pink-500/10"}`}>
                      <Heart className={`size-4 ${likedClips.has(clip.id) ? "fill-current" : ""}`} /> {clip.likes + (likedClips.has(clip.id) ? 1 : 0)}
                    </button>
          ...
          <>
            <aside className={`${showCommunityPane ? "hidden lg:flex" : "flex"} w-full lg:w-80 xl:w-96 shrink-0 flex-col border-r border-border/60 bg-surface/20`}>
              <div className="px-4 py-3 border-b border-border/60 bg-surface/40 backdrop-blur-xl flex items-center justify-between shrink-0">
                ...
                <button onClick={() => setShowCreateCommunity(true)} className="size-9 rounded-xl grid place-items-center text-muted-foreground hover:text-foreground hover:bg-surface-2 transition">
                  <Plus className="size-4.5" />
                </button>
              </div>
              <div className="px-3 py-2.5 border-b border-border/40 shrink-0">
                <div className="relative">
                  <Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" />
                  <Input placeholder="Search communities…" className="pl-9 bg-surface-2/50 border-border/40 rounded-xl h-9 text-sm" />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto">
                ...
                {DEMO_COMMUNITIES.filter((c) => joinedCommunities.has(c.id)).map((c) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 transition border-b border-border/20 ${activeCommunity?.id === c.id ? "bg-surface-2/70" : "hover:bg-surface-2/40"}`}
                  >
              ...
              {!activeCommunity ? (
                <div className="flex-1 grid place-items-center text-center px-6">
                  <div className="max-w-xs">
                    ...
                    <Button className="mt-4 rounded-xl" size="sm" onClick={() => setShowCreateCommunity(true)}>
                ...
                <>
                  <header className="flex items-center gap-3 px-4 h-[62px] border-b border-border/60 bg-surface/50 backdrop-blur-xl shrink-0">
                    <button className="lg:hidden size-9 rounded-lg grid place-items-center text-muted-foreground hover:bg-surface-2 mr-1" onClick={() => setActiveCommunity(null)}>
                      <ArrowLeft className="size-4.5" />
                    </button>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => { setJoinedCommunities((prev) => { const next = new Set(prev); next.delete(activeCommunity.id); return next; }); setActiveCommunity(null); toast.success("Left community"); }} className="text-red-400 focus:text-red-400">
                            <LogOut className="size-3.5 mr-2" /> Exit community
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  ...
                  <div className="shrink-0 flex gap-2 px-4 py-2.5 border-b border-border/40 overflow-x-auto scrollbar-none">
                    {activeCommunity.channels.map((ch) => (
                      <button key={ch} onClick={() => setActiveChannel(ch)} className={`shrink-0 flex items-center gap-1.5 px-3 h-7 rounded-full text-xs font-medium transition ${activeChannel === ch ? "bg-primary text-primary-foreground" : "bg-surface-2/60 text-muted-foreground hover:bg-surface-2 hover:text-foreground"}`}>
                        <Hash className="size-3" />{ch}
                      </button>
                    ))}
                  </div>
                  <div className="flex-1 grid place-items-center text-center px-6">
